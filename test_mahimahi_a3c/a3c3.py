''' improved a3c for python 2

    wider FC layer / number of kernels in input layers (split 0 - 5)
...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            WIDTH = 256 # 128
            split_0 = tflearn.fully_connected(inputs[:, 0:1, -1], WIDTH, activation='relu')
            split_1 = tflearn.fully_connected(inputs[:, 1:2, -1], WIDTH, activation='relu')
            split_2 = tflearn.conv_1d(inputs[:, 2:3, :], WIDTH, 4, activation='relu')
            split_3 = tflearn.conv_1d(inputs[:, 3:4, :], WIDTH, 4, activation='relu')
            split_4 = tflearn.conv_1d(inputs[:, 4:5, :A_DIM], WIDTH, 4, activation='relu')
            split_5 = tflearn.fully_connected(inputs[:, 4:5, -1], WIDTH, activation='relu')
