...
# hagay
# global data collectors for training
...
train_s_batch = [np.zeros((S_INFO, S_LEN))]
train_a_batch = [action_vec]
train_r_batch = [0]
train_entropy_record = []
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                # hagay
                train_entropy_record.append(a3c.compute_entropy(action_prob[0]))
                ###############################
                ...
                if end_of_video:
                    ...
                    train_action_vec = np.zeros(A_DIM)
                    train_action_vec[bit_rate] = 1

                    train_s_batch.append(np.zeros((S_INFO, S_LEN)))
                    train_a_batch.append(train_action_vec)

                else:
                    train_s_batch.append(state)

                    action_vec = np.zeros(A_DIM)
                    action_vec[bit_rate] = 1
                    train_a_batch.append(action_vec)
                #####################################################################
...
if __name__ == "__main__":
    try:
        ...
        print("save train collected data")
        import pickle
        filename = './train_data_' + str(process_id)
        # assume ended because end of video
        end_of_video = 1
        train_data = ([train_s_batch[1:],  # ignore the first chuck
                       train_a_batch[1:],  # since we don't have the
                       train_r_batch[1:],  # control over it
                       end_of_video,
                       {'entropy': train_entropy_record}])
